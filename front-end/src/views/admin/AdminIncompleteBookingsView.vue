    ...
    <main class="flex-1 lg:ml-64 bg-neutral-50 min-h-screen w-full overflow-x-hidden">
      ...
      <header class="bg-white border-b border-neutral-100 px-4 sm:px-6 py-4 sticky top-0 z-10">
        <div class="ml-14 lg:ml-0">
          <h1 class="text-xl font-semibold text-neutral-900">Incomplete Bookings</h1>
          <p class="text-sm text-neutral-500">Accepted but not yet completed</p>
        </div>
      </header>
      ...
      <div class="p-4 sm:p-6">
        ...
        <div class="bg-white rounded-lg border border-neutral-200 p-4 mb-6">
          <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
            <div>
              <label class="form-label">Days Since Acceptance</label>
              ...
              <table class="w-full">
                <thead class="bg-neutral-50 border-b border-neutral-200">
                  <tr>
                    ...
                    <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Days Pending</th>
                ...
                <tbody class="divide-y divide-neutral-200">
                  <tr v-for="booking in bookings" :key="booking.Booking_ID" class="hover:bg-neutral-50">
                    ...
                    <td class="px-4 py-3 text-sm">
                      <span class="px-2 py-1 bg-info-100 text-info-700 rounded text-xs font-medium capitalize">
                        {{ booking.Status }}
                      </span>
                    </td>
                    <td class="px-4 py-3 text-sm">
                      <span class="px-2 py-1 bg-warning-100 text-warning-700 rounded text-xs font-medium">
                        {{ booking.Days_Since_Acceptance }}
                      </span>
                    </td>
        ...
        <div v-if="bookings.length > 0" class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          <div class="card">
            <div class="card-body">
              ...
              <p class="text-2xl font-bold text-neutral-900">${{ totalValue }}</p>
            </div>
          </div>
          <div class="card">
            <div class="card-body">
          ...
          <div class="card">
            <div class="card-body">
              <p class="text-sm text-neutral-500 mb-1">Avg Days Pending</p>
              <p class="text-2xl font-bold text-neutral-900">{{ avgDaysPending }}</p>
            </div>
          </div>
        </div>
      </div>
...
export default {
  name: 'AdminIncompleteBookingsView',
  ...
  computed: {
    ...
    avgDaysPending() {
      ...
      const sum = this.bookings.reduce((total, b) => total + (b.Days_Since_Acceptance || 0), 0)
  ...
  methods: {
    async loadBookings() {
      ...
      try {
        const result = await adminStore.fetchAcceptedIncomplete(parseInt(this.daysFilter))
