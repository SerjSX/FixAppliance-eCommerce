    ...
    <main class="flex-1 lg:ml-64 bg-neutral-50 min-h-screen w-full overflow-x-hidden">
      ...
      <header class="bg-white border-b border-neutral-100 px-4 sm:px-6 py-4 sticky top-0 z-10">
        <div class="ml-14 lg:ml-0">
          <h1 class="text-xl font-semibold text-neutral-900">Completed Unpaid Bookings</h1>
          <p class="text-sm text-neutral-500">Bookings completed but payment not received</p>
        </div>
      </header>
      ...
      <div class="p-4 sm:p-6">
        ...
        <div class="bg-white rounded-lg border border-neutral-200 p-4 mb-6">
          <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
            <div>
              <label class="form-label">Days Since Completion</label>
              <select v-model="daysFilter" class="form-input">
              ...
              <table class="w-full">
                <thead class="bg-neutral-50 border-b border-neutral-200">
                  <tr>
                    ...
                    <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Appliance</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Amount</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-700">Days Unpaid</th>
                ...
                <tbody class="divide-y divide-neutral-200">
                  <tr v-for="booking in bookings" :key="booking.Booking_ID" class="hover:bg-neutral-50">
                    ...
                    <td class="px-4 py-3 text-sm text-neutral-600">{{ booking.Appliance_Type }}</td>
                    <td class="px-4 py-3 text-sm font-semibold text-neutral-900">${{ booking.Total_Price }}</td>
                    <td class="px-4 py-3 text-sm">
                      <span class="px-2 py-1 bg-error-100 text-error-700 rounded text-xs font-medium">
                        {{ booking.Days_Since_Completion }}
                      </span>
                    </td>
                    <td class="px-4 py-3 text-sm">
                      <button class="text-primary-600 hover:underline text-xs font-medium">
                        View Details
                      </button>
                    </td>
                  </tr>
        ...
        <div v-if="bookings.length > 0" class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          <div class="card">
            <div class="card-body">
              <p class="text-sm text-neutral-500 mb-1">Total Unpaid</p>
              <p class="text-2xl font-bold text-error-600">${{ totalUnpaid }}</p>
            </div>
          </div>
          <div class="card">
            <div class="card-body">
          ...
          <div class="card">
            <div class="card-body">
              <p class="text-sm text-neutral-500 mb-1">Avg Days Unpaid</p>
              <p class="text-2xl font-bold text-neutral-900">{{ avgDaysUnpaid }}</p>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    totalUnpaid() {
      return this.bookings.reduce((sum, b) => sum + (b.Total_Price || 0), 0).toFixed(2)
    },
    avgDaysUnpaid() {
      if (this.bookings.length === 0) return 0
      const sum = this.bookings.reduce((total, b) => total + (b.Days_Since_Completion || 0), 0)
  ...
  methods: {
    async loadBookings() {
      ...
      try {
        const result = await adminStore.fetchCompletedUnpaid(parseInt(this.daysFilter))
