  ...
  <div class="min-h-screen flex">
    ...
    <aside class="fixed left-0 top-0 bottom-0 w-64 bg-primary-800 text-white overflow-y-auto" role="navigation" aria-label="Technician navigation">
      ...
      <nav class="p-4 space-y-1">
        ...
        <router-link to="/technician/pending-bookings" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary-700 text-white">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
          </svg>
          <span>Pending Bookings</span>
          <span v-if="pendingBookings.length > 0" class="ml-auto bg-secondary-500 text-white text-xs px-2 py-0.5 rounded-full">{{ pendingBookings.length }}</span>
        </router-link>
            ...
            <div class="card-body">
              <div class="flex flex-col md:flex-row md:items-start gap-4">
                ...
                <div class="flex gap-2">
                  ...
                  <button @click="declineBooking(booking.id)" class="btn btn-outline btn-sm text-error-600">Decline</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async declineBooking(bookingId) {
      await useTechnicianStore().declineBooking(bookingId)
    }
  },
  async mounted() {
    this.loading = true
